...
const std::string MULTICAST_IP = "224.2.2.2";
    ...
    do
    {
      ...
      if( numBytes > 0 )
      {
        ...
        if (header == NAT_FRAMEOFDATA && version)
        {
          ...
          {
            ROS_DEBUG("Num %d",format.model.numRigidBodies);
            count_mode++;
            ...
            if(count_mode>100000)count_mode=0;
          }
        }
        
        if (header == NAT_PINGRESPONSE) {
          ROS_INFO("Header : %d, %d", header, PacketIn.iMessage);
          ROS_INFO("nData : %d", PacketIn.nDataBytes);
